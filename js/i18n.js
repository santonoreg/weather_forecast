...
const I18N = {
  en: {
    ...
    'r.legend': 'Verification for {start} – {end} ({hours} hours) at "{loc}". Each model\'s archived forecasts (Open-Meteo Historical Forecast) are compared with the ERA5 reanalysis. The score (0–100) is the average skill across parameters and yields the weights (0.5–1.8) that weight the forecast when weighting is enabled.',
    'r.limits': '<b>Limitations:</b> ERA5 is not a station measurement and is produced by the ECMWF model, so it slightly favours ECMWF. Archived forecasts mostly reflect short lead times. Models that do not cover the area (e.g. KNMI, DMI, MET Norway Nordic) and Yr are not scored and count with weight 1. Rain detection depends on how many rainy days occurred in the period.',
